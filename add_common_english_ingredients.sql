-- Add common English ingredients to the database
-- This script adds a comprehensive list of common cooking ingredients

-- First, let's check if the unique constraint exists, if not create it
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint 
        WHERE conname = 'ingredients_name_unique_idx'
    ) THEN
...
SELECT 
  'Total ingredients added' as summary,
  COUNT(*) as count
